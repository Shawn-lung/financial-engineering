...
def trade_logic(data, portfolio):
    ...
    for date in data['2330.TW'].index:
        ...
        if not initial_invested:
            available_tickers = [ticker for ticker in ["TLT", "GLD"] if prices.get(ticker) is not None]
            if len(available_tickers) == len(["TLT", "GLD"]):
                print(f"Initial investment on {date}")
                for ticker in available_tickers:
                    allocation = portfolio.allocation[ticker]
                    amount_to_buy = (portfolio.cash * allocation) / prices[ticker]
                    portfolio.buy(date, ticker, prices[ticker], amount_to_buy)
                    print(f"Bought {amount_to_buy} of {ticker} at {prices[ticker]}")
                initial_invested = True
            else:
                print(f"Skipping initial investment on {date} due to missing data for required tickers")
                ...
                if ticker in ["2330.TW", "4763.TW", "8069.TWO", "2881.TW", "2882.TW"]:
                    if (df.loc[date, 'EMA20'] > df.loc[date, 'EMA60'] and 
                        ...
                        if ticker not in portfolio.entry_prices:  # 初始进场
                            ...
                            increase = (current_price/entry_price - 1) *100
                            ATR = df['ATR'].loc[date]
                            if current_price <= entry_price - ATR * 2:  # 退场策略
                                ...
                                print(f"Sold {amount_to_sell} of {ticker} at {current_price} on {date} due to ATR stop-loss : {increase}%")
                                ...
                                if current_price >= entry_price * 1.05:  # 上涨5%加仓
                                    ...
                                    print(f"Add Buy: {amount_to_buy} of {ticker} at {current_price} on {date} due to 5% rise: {increase}%")
                                elif current_price <= entry_price * 0.90:  # 下跌10%加仓
                                    amount_to_buy = (portfolio.cash * portfolio.allocation[ticker] * 0.25) / current_price
                                    portfolio.buy(date, ticker, current_price, amount_to_buy)
                                    portfolio.entry_prices[ticker] = current_price  # 重置entry_price
                                    portfolio.count[ticker] += 1
                                    print(f"Add Buy: {amount_to_buy} of {ticker} at {current_price} on {date} due to 10% fall: {increase}%")
